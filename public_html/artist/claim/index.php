...
if (empty($_SESSION['creator_claim_csrf'])) {
    $_SESSION['creator_claim_csrf'] = bin2hex(random_bytes(16));
}

$prefillName = trim((string)($_GET['name'] ?? ''));

$pageTitleText = ($lang === 'en')
  ? 'iceBeats.io – Claim artist profile'
  : 'iceBeats.io – Verifiera artistprofil';

$pageDescText = ($lang === 'en')
  ? 'Claim your artist profile on iceBeats.io and request access to edit it.'
  : 'Verifiera din artistprofil på iceBeats.io och begär åtkomst för att redigera den.';

$canonical = ij_abs('/artist/claim/');
if ($prefillName !== '') {
    $canonical .= '?name=' . rawurlencode($prefillName) . '&lang=' . rawurlencode($lang);
} else {
    $canonical .= '?lang=' . rawurlencode($lang);
}
...
$T = [
  'sv' => [
    'h1' => 'Verifiera artistprofil',
    'sub' => 'Skicka in en begäran om att göra anspråk på en artistprofil. När den godkänts kan du få en redigeringslänk via e-post.',
    'creator' => 'Artist',
    'name' => 'Ditt namn',
    'email' => 'Din e-post',
    'proof' => 'Bevislänk',
    'proof_hint' => 'T.ex. Spotify, Instagram, Suno-profil eller annan länk som visar kopplingen till artistnamnet.',
    'message' => 'Meddelande',
    'ph_creator' => 't.ex. Legendai',
    'ph_name' => 'Ditt namn',
    'ph_email' => 'din@email.se',
    'ph_proof' => 'https://...',
    'ph_message' => 'Beskriv kort varför du ska ha tillgång till profilen…',
    'send' => 'Skicka verifiering',
    'sending' => 'Skickar…',
    'required' => 'Fyll i artist, namn och e-post.',
    'email_invalid' => 'Ange en giltig e-postadress.',
    'success' => 'Tack! Din verifiering har skickats in och väntar på granskning.',
    'error' => 'Något gick fel. Försök igen.',
    'wait' => 'Vänta',
    'note' => 'Vi granskar verifieringar manuellt innan en profil kopplas till en artist.',
    'back_creator' => 'Till artist-sidan',
    'open_library' => 'Öppna Låtar',
  ],
  'en' => [
    'h1' => 'Claim artist profile',
    'sub' => 'Submit a request to claim an artist profile. Once approved, you can receive an edit link by email.',
    'creator' => 'Artist',
    'name' => 'Your name',
    'email' => 'Your email',
    'proof' => 'Proof URL',
    'proof_hint' => 'For example Spotify, Instagram, Suno profile or another link showing the connection to the artist name.',
    'message' => 'Message',
    'ph_creator' => 'e.g. Legendai',
    'ph_name' => 'Your name',
    'ph_email' => 'your@email.com',
    'ph_proof' => 'https://...',
    'ph_message' => 'Briefly explain why you should get access to the profile…',
    'send' => 'Send claim',
    'sending' => 'Sending…',
    'required' => 'Please fill in artist, name and email.',
    'email_invalid' => 'Please enter a valid email address.',
    'success' => 'Thanks! Your claim has been submitted and is awaiting review.',
    'error' => 'Something went wrong. Please try again.',
    'wait' => 'Wait',
    'note' => 'Claims are reviewed manually before a profile is connected to a artist.',
    'back_creator' => 'Back to artist page',
    'open_library' => 'Open Tracks',
  ],
];
  ...
  <div class="card">
    <div class="formWrap">
      <form id="claimForm" novalidate>
        ...
        <input type="hidden" name="csrf" value="<?= h($_SESSION['creator_claim_csrf']) ?>">
        ...
        <div class="field">
          <label class="label" for="proof_url"><?= h($L['proof']) ?></label>
          <input class="input" type="url" id="proof_url" name="proof_url" placeholder="<?= h($L['ph_proof']) ?>">
          <div class="help"><?= h($L['proof_hint']) ?></div>
        </div>

        <div class="field">
          <label class="label" for="message"><?= h($L['message']) ?></label>
          <textarea class="textarea" id="message" name="message" placeholder="<?= h($L['ph_message']) ?>"></textarea>
        </div>
        ...
        <div class="help" style="margin-top:14px;"><?= h($L['note']) ?></div>
      </form>
    </div>
  </div>
</div>
...
form.addEventListener('submit', async (e) => {
  ...
  try {
    const res = await fetch('/api/creator_claim.php', {
      method: 'POST',
      body: fd,
      cache: 'no-store',
    ...
    form.reset();
    document.getElementById('creator_name').value = creator;
    document.getElementById('form_loaded_at').value = String(Math.floor(Date.now() / 1000));
